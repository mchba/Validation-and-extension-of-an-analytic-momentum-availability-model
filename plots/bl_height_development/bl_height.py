...
h_tauw0 = np.array(h_tauw0) # 5% based on tauw0
h_tauw1 = np.array(h_tauw1) # 5% based on tauw1
...
herror1 = (hratModel-hratLES1)/hratLES1*100
...
for i in range(len(H)):
    ...
    pcl.append(pcl0)
    # Theoretical model
    ax[i].plot(lines[i].x/1000, h0[i]*UF0[i]/lines[i]['u']/1000, '--', color='k')
    # Mark farm region
    ax[i].axvspan(cvxstart/1000, cvxend/1000, color='gray', alpha=0.4)
    ax[i].text(22, 1.05, r'Error: %.1f'%(herror0[i]) + '%', fontsize=15,
