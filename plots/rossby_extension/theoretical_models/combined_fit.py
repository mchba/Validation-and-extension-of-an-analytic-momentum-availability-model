...
###############################################
# Lanzilao 2024
with open('../power_function_fits/lanzilao2024fit.pkl', 'rb') as fp:
...
# Left plot
ax1.set_xlabel(r'$Ro_{h0}^{-1}=f_c h_0/G$')
ax1.set_ylabel(r'$h_{x0}/h_0$',**yd)
# Plot Lanzilao2024 data
for i in range(len(data_l2024['h0fit'])):
    ax1.plot(1/data_l2024['Ro_h0'][i], data_l2024['h0fit_taux'][i]/data_l2024['h0fit'][i], marker='s', color=colors[i],
...
# Right plot
ax2.set_xlabel(r'$Ro_{h0}^{-1}=f_c h_0/G$')
ax2.set_ylabel(r'$p_{x0}$',**yd)
# Plot Lanzilao2024 data
for i in range(len(data_l2024['h0fit'])):
    ax2.plot(1/data_l2024['Ro_h0'][i], data_l2024['p_fit_taux'][i], marker='s', color=colors[i],
            label='H%d'%(H[i]), markersize=8, linestyle='None',zorder=99)
# Plot Berg2020 data
