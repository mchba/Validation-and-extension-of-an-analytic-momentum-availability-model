...
'''
    Calculate ABL height (only precursor!) for different definitions of
...
# uw and vw (raw)
fig, axes = base_plot(xlim=[-0.03,0.1])
for i in range(len(H)):    
    precursori = precursors[i]
...
axes[0].set_title(r'$\tau_x$ - Raw precursor',fontsize=17)
axes[1].set_title(r'$\tau_y$ - Raw precursor',fontsize=17)
for ax in axes:
    ax.set_ylim([0,1000])
...
# uw and vw (zoom-in with linear wall extrapolation)
...
for i in range(len(H)):
    ...
    axes[0].plot((1 - z_ext0[i]/h0fit[i])**(p_fit[i]), z_ext0[i], '--', color=colors[i],
                 label=r'Fit: $p_0$=%.2f, $h_0$=%.0f m' % (p_fit[i], h0fit[i]))
    ...
    axes[1].plot((1 - z_ext0[i]/h0fit_taux[i])**(p_fit_taux[i]), z_ext0[i], '--', color=colors[i],
                 label=r'Fit: $p_{x0}$=%.2f, $h_{x0}$=%.0f m' % (p_fit_taux[i], h0fit_taux[i]))
for ax in axes:
    ax.set_ylim([0,1250])
    ax.set_xlim([0,1])
...
if True:
    ...
    with open('lanzilao2024fit.pkl', 'wb') as fp:
