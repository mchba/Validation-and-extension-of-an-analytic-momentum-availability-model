...
## LOAD LES DATA ##########################################################
###########################################################################
# LOAD WIND FARM LAYOUT (same layout for all cases) #################################
...
p0 = getList(precursors,'p0')
    


## Load turbine data (we need the total thrust of the farm)
turbines = []
for i in range(len(H)):
    sim_name = 'H%d-C5-G4'%(H[i])
    with open('../../data/' + sim_name + '/turbines_avg.pkl', 'rb') as fp:
...
## Load BL height over farm
h_tauw0 = []
h_tauw1 = []
for i in range(len(H)):
    sim_name = 'H%d-C5-G4'%(H[i])
    with open('../../data/' + sim_name + '/bl_height.pkl', 'rb') as fp:
        bldata = pickle.load(fp)
        h_tauw0.append(bldata['h_tauw0'])
        h_tauw1.append(bldata['h_tauw1'])
h_tauw0 = np.array(h_tauw0)
h_tauw1 = np.array(h_tauw1)



## LOAD 3D FLOW FIELDS IN CV REGION #################################
datasets = []
for i in range(len(H)):
    sim_name = 'H%d-C5-G4'%(H[i])
    # Load dataset
    datai = xarray.load_dataset('../../data/' + sim_name+"/main_cv.nc")
    # Add to list
    datasets.append(datai)   
    

## Load beta and pressure data (used for model comparison)
beta_datasets = []
for i in range(len(H)):
    sim_name = 'H%d-C5-G4'%(H[i])
...
beta0 = getList(beta_datasets,'beta0')
betaL = getList(beta_datasets,'betaL')
Pin = getList(beta_datasets,'Pin')
...
ax[1].text(1.1,2.75,r"%.1f"%(madv_error[0]) + "%",ha='center', fontsize=13)
ax[1].text(2.1,2.4,r"%.1f"%(madv_error[1]) + "%",ha='center', fontsize=13)
ax[1].text(3.1,4.0,"+" + r"%.1f"%(madv_error[2]) + "%",ha='center', fontsize=13)
ax[1].set_ylim([0,5])
...
ax[1].text(1.1,5.9,"+" + r"%.1f"%(mPGF_error[0]) + "%",ha='center', fontsize=13)
ax[1].text(2.1,4.8,"+" + r"%.1f"%(mPGF_error[1]) + "%",ha='center', fontsize=13)
ax[1].text(3.1,3.1,r"%.1f"%(mPGF_error[2]) + "%",ha='center', fontsize=13)
ax[1].set_ylim([0,8])
fig.text(0.14,0.92,r"$\bf{(a)}$",ha='center', fontsize=15)
fig.text(0.75,0.92,r"$\bf{(b)}$",ha='center', fontsize=15)
...
mPGF_error = (mdeltaM_PGFnorm-deltaM_PGFnorm)/deltaM_PGFnorm*100
ax[1].text(1.1,8.5,r"%.1f"%(sa_Merror[0]) + "%",ha='center', fontsize=13)
ax[1].text(2.1,7.2,r"%.1f"%(sa_Merror[1]) + "%",ha='center', fontsize=13)
ax[1].text(3.1,6.8,r"%.1f"%(sa_Merror[2]) + "%",ha='center', fontsize=13)
ax[1].set_ylim([0,13])
fig.text(0.14,0.92,r"$\bf{(a)}$",ha='center', fontsize=15)
...
## Test of model (with different settings) #########################
M3 = msadeltaM_advpgfnorm             # Model of (Madv+Mpgf)/MF0 with small approximation
h1 = h005/h_tauw1                     # Exact h0/h (used in Fig.13b in article of shear stress profiles)
h2 = beta                             # Model h0/h
...
# Mturb bars figure for article
fig = plt.figure(figsize=(9, 4))
gs = gridspec.GridSpec(1, 3)
gs.update(wspace=1)
ax = [plt.subplot(gs[0, :2]),  plt.subplot(gs[0, 2:])]
ax[0].plot(H,h1,'ks-',color=adv_colors[0], label=r'LES ($h$ based on 5% of $\tau_{w1}$)')
ax[0].plot(H,h005/h_tauw0,'ks:',color=adv_colors[0], label=r'LES ($h$ based on 5% of $\tau_{w0}$)')
...
ax[1].text(1.1,3.5,"+" + r"%.0f"%(mstr_error[0]) + "%",ha='center', fontsize=13)
ax[1].text(2.1,5.0,"+" + r"%.0f"%(mstr_error[1]) + "%",ha='center', fontsize=13)
ax[1].text(3.1,5.9,r"%.0f"%(mstr_error[2]) + "%",ha='center', fontsize=13)
ax[1].set_ylim([0,8])
fig.text(0.14,0.92,r"$\bf{(a)}$",ha='center', fontsize=15)
fig.text(0.75,0.92,r"$\bf{(b)}$",ha='center', fontsize=15)
...
M_KDN2_i_and_ii = 1 + 1.18*(1-beta) + h0/(L*Cf0)*(2.18*(1-beta))                      # Add linearization (i) and (ii)
...
##################################################################################
########## Print LaTex table data (table 1 in article) ###########################
##################################################################################
print(' -------- Table 1 ------------')
for i in range(len(H)):
    print('H%d'%(H[i]) + ' & ' +
          ...
          '$%.2f$'%(UF0[i]/G) + ' & ' +                    # UF0/G
          '$%.5f$'%(Cf0[i]) + ' & ' +                      # Cf0
          '$%.3f$'%(taut0[i]/tauw0[i]) + '\\' +'\\')       # taut0/tauw0
print(' ')
